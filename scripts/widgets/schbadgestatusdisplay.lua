...
local HealthBadge = require "widgets/schwarlockbadge"
local HungerBadge = require "widgets/schwarlockbadge"
--local SanityBadge = require "widgets/sanitybadge"

local SchiBadgeStatusDisplay = Class(Widget, function(self, owner)
...
function SchiBadgeStatusDisplay:UpdateValues()
...
function SchiBadgeStatusDisplay:OnUpdate()

	local target = nil

	local ents = TheInput:GetAllEntitiesUnderMouse()
	for k,v in pairs(ents) do
		if v:HasTag("Schwarzkirsche") and v.Transform then
			target = v
			break
		end
	end
...
function SchiBadgeStatusDisplay:UpdatePosition(x,y)
    self:SetPosition(x+75,y-10,0)
end

function SchiBadgeStatusDisplay:FollowMouseConstrained()
...
function SchiBadgeStatusDisplay:HealthDelta(data)
...
function SchiBadgeStatusDisplay:HungerDelta(data)
