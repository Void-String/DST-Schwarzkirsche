...
local REPEL_RADIUS = .25
local REPEL_RADIUS_SQ = REPEL_RADIUS
...
local function MakeShield(name, num, prefabs)
    local function fn()
        ...
        inst.AnimState:SetBuild("sch_stalker_shield")
...
local prefs = {}
for i = 1, 4 do
    local name = "sch_stalker_shield_mod"..tostring(i)
    table.insert(prefs, name)
    table.insert(ret, MakeShield(name, i))
end
table.insert(ret, MakeShield("sch_stalker_shield_mod", nil, prefs))
prefs = nil
