local assets =
{
    Asset("ANIM", "anim/sch_sand_spike.zip"),
    Asset("ANIM", "anim/sch_sand_splash_fx.zip"),
}

local block_assets =
{
    Asset("ANIM", "anim/sch_sand_block.zip"),
    Asset("ANIM", "anim/sch_sand_splash_fx.zip"),
}
...
local NON_COLLAPSIBLE_TAGS = { "schwarz", "player", "groundspike", "flying", "shadow", "ghost", "playerghost", "FX", "NOCLICK", "DECOR", "INLIMBO" }
...
local function MakeSpikeFn(shape, size)
    return function()
        ...
        if shape == "spike" then
            ...
            if size == nil then
                inst:SetPrefabName("sch_sandspike_"..inst.spikesize)
            end
            inst:SetPrefabNameOverride("schsandspike")
        else
            inst.animname = "block"
        end
        ...
        inst.AnimState:SetBuild("sch_sand_"..shape)
...
for i, v in ipairs(SPIKE_SIZES) do
    local name = "sch_sandspike_"..v
    table.insert(prefabs, name)
    table.insert(ret, Prefab(name, MakeSpikeFn("spike", v), assets, { "glass_fx", "glassspike_"..v }))
end
table.insert(ret, Prefab("schsandspike", MakeSpikeFn("spike"), assets, prefabs))
prefabs = nil

table.insert(ret, Prefab("schsandblock", MakeSpikeFn("block"), block_assets, { "glass_fx", "glassblock" }))


----- Lua Prefab
table.insert(ret, Prefab("sch_sandfx", MakeSpikeFn("block"), block_assets, { "glass_fx", "glassblock" }))
